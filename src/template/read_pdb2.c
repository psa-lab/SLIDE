/** Functions for reading in a pdb file  and store the atoms in the 
    pdb_atoms_pt data structure **/
...
/** reads a pdb file and stores the atoms in an array. returns the total number
of atoms found **/

int  read_pdb2 ( char  *filename, pdb_atom_pt pdb_atoms )
{
	FILE         *in;
	pdb_atom_pt  atom;
	...
	while ( fgets ( linebuffer, MAX_LINE_LENGTH, in ) != NULL )
		if ( strncmp ( linebuffer, "ATOM", 4 ) == 0 )
		{
			...
			atom->chainID= linebuffer[21];
			atom->icode= linebuffer[26];

			atom->pos[X] = atof ( &linebuffer[30] );
			atom->pos[Y] = atof ( &linebuffer[38] );
			atom->pos[Z] = atof ( &linebuffer[46] );
			if ( ( strncmp ( atom->atom_name, "N", 1 ) == 0 
			    && strncmp ( atom->residue_name, "PRO", 3 ) != 0 )
			    || strncmp ( atom->atom_name, "O", 1 ) == 0 )
			{
				if ( strncmp ( atom->atom_name, "N", 1 ) == 0 )
					atom->act = DONOR;
				if ( strncmp ( atom->atom_name, "O", 1 ) == 0 )
					atom->act = ACCEPTOR;
				if ( strncmp ( atom->atom_name, "OG", 2 ) == 0 
				    || strncmp ( atom->atom_name, "OH", 2 ) == 0 )
					atom->act = DONEPTOR;
			}
			else
				atom->act = NOTHING;
		   ...
		   *column ( 19 ) == 'C' && *column ( 20 ) == 'A' )
		 {
	      ...
	      /******************* end of metal check ********************/
				...
				if ( strncmp ( atom->atom_name, "N", 1 ) == 0 
				    || strncmp ( atom->atom_name, "O", 1 ) == 0 )
				{
					if ( strncmp ( atom->atom_name + 1, "DD", 2 ) == 0 )
						atom->act = DONOR;
					if ( strncmp ( atom->atom_name + 1, "AA", 2 ) == 0 )
						atom->act = ACCEPTOR;
					if ( strncmp ( atom->atom_name + 1, "NN", 2 ) == 0 )
						atom->act = DONEPTOR;
				}
				if ( strncmp ( atom->atom_name, "N", 1 ) == 0 )
					atom->hphil = 350;
				if ( strncmp ( atom->atom_name, "O", 1 ) == 0 )
					atom->hphil = 530;
				if ( strncmp ( atom->atom_name, "C", 1 ) == 0 )
					atom->hphil = 80;
				atom++;
				number_of_atoms++;
				if ( number_of_atoms == MAX_NUMBER_OF_ATOMS )
