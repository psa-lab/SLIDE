				...
				double    threshold )
{
  ...
  /* get the mininum distance between any two points */
  for ( i = 0; i < number_of_points; i++ )
    if ( marker[i] == IN )
	...
	if ( marker[j] == IN )
	  {
	    ...
	    if ( dist < min_dist && ( (dist - min_dist) > MIN_DOUBLE || (dist - min_dist) < -MIN_DOUBLE) )
	      {
		min_dist = dist;
		min_i = i;
		min_j = j;
  ...
  for ( i = 0; i < number_of_points; i++ )
	...
	if ( number_of_clusters >= MAX_NUMBER_OF_TEMPLATE_POINTS )
