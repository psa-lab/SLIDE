...
/* define the number of buckets in the type hash table -- most parameters
 * are now handled in ~slide/params/slide.parameters */
...
/* The squared distances +/- tolerances are used to keep this version of 
 * SLIDE to be as close as possible to SLIDE 3.0.2.  In the future if anyone
 * has ample time and desire it would be better to use careful rounding
 * or the same tolerance everywhere possible.
 */
...
/* add by Litian to this special version of slide  --08/03/2003
   to calculate the score after we got weights from each parameter in
   the scoring funcions defined in src/slide/scoring/score_define.txt */
