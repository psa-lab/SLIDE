    ...
    if(global->unbump_bonds[bond] >= 0){
      ...
      if(total_intra_overlap > 1.0){
       ...
       printf("Intra overlap of %5.3lf, rotation not accepted\n", 
        ...
        /* now update the distances for the rotated atoms in the lookup table
         * 
         * 08MAY06 toneroma changed all dist_fun_cut to dist_fun to check 
         * "rounding error" 
         *   = (float)  dist_fun_cut ( target[atoms_to_rotate[k]].pos, ligand[j].pos ); 
         */
