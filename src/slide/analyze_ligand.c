...
int  analyze_ligand ( global_data_pt  global )
{
  ...
  for ( i = 0; i < compounds[global->compound_index].number_of_points; i++ ){
    /* Look at polar atoms here */
    if ( act[first+i] == HYDROPHOB ) continue;
      
    /* the index of the donor and acceptors is based on the numbering
     * in the mol2, which consideres dummy atoms, so find the real
     * index used internally for this atom */
