/*
 * $Source: /psa/share/repository/slide/src/slide/read_parameter_file.c,v $
 * $Revision: 1.13 $
 * $Author: toneroma $
 ...
 * fixed problem with match_2_key_points and group_conformers where slide.parameters in the /in/ directory wouldn't be used
 *
 * Revision 1.8  2007/09/28 18:33:48  toneroma
 * *** empty log message ***
 *
 * Revision 1.7  2007/02/08 19:51:33  toneroma
 * added error messages for using the incorrect slide.parameters file
 *
 ...
 * Moved the more delicate parameters to inc/defs.H from slide.parameters.
 *
 ...
 * fields.  For example in the slide.parameters file the unit field may not
  ...
  if(rv == EOF)
    {
      sprintf(err_msg, "Error when scanning the line:\n\t%sin the "
	      "slide.parameters file: %s"
	      "Make sure the parameters file matches the format of the default parameters file in:\n"
	      "%s/params/slide.parameters\n\n", line, strerror(errno), getenv("SLIDE_DIR"));
      err_print(err_msg);
...
int read_parameter_file(global_data_pt global, char *file )
{
  ...
  while ( fgets ( line, MAX_PARAMETER_LINELENGTH, fp ) )
    ...
    case RESTART_MOLECULE:
      ...
      if((!strcmp(value, "NONE")) || (!strcmp(value, "None")) || (!strcmp(value, "none"))) global->restart_molecule_check = FALSE;
        ...
        printf("NOTE: SLIDE run restarting at molecule: %s.\n", value);
        global->restart_molecule_check = TRUE;
      }
      break;
    case INVALID_ENTRY_OR_ERROR:
      sscanf(line, "%s %s %s %*s", variable, unit, value);
      sprintf(err_msg, "\n * INVALID SLIDE.PARAMETERS LINE * \nparameter: %s\n"
              "unit: %s\nvalue: %s\n"
	    "Make sure the parameters file matches the format of the default parameters file in:\n"
	    "%s/params/slide.parameters\n\n", variable, unit, value, getenv("SLIDE_DIR") );
      err_print(err_msg);
      fprintf(stderr, err_msg);
      printf(err_msg);
      exit(1);
  ...
  if ( scan_parameter_file_line ( line, value, global) != NUM_PARAMETER_TYPES )
    {
      sprintf(err_msg, "\n * INCORRECT # OF LINES IN slide.paramters * \n"
	      "Make sure the parameters file matches the format of the default parameters file in:\n"
	      "%s/params/slide.parameters\n\n", getenv("SLIDE_DIR") );
      err_print(err_msg);
      fprintf(stderr, err_msg);
