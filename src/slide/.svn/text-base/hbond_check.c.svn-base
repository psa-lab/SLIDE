				...
				global_data_pt global)
{
  ...
  if(count > 1){ 
    for(i = 0; i < 3; i++){
	/*      ypoint[i] /= count;  toneroma 07JUN06 debugging comment out*/
          /* More obfuscated code - if we want mid point just take the average
           * of the two heavy atom positions rather than divide by (count - 1)
           *
           * Anyhow the idea should be either to set the rigid hydrogen
           * positions after all of the rotatable bonds are resolved or 
           * they should be set here.  The issue is we use the hydrogen atoms
           * during bump check -- thus, they should be set before and during 
           * bump check.  We should not move the rigid hydrogens here.  This is
           * a bit subtle in the sense that we should either respect bumping or
           * not.
           */
      ypoint[i] -= xpoint[i];
      ypoint[i] /= (count - 1);
  ...
  /* only one neighbor, so we just assume that the hydrogen is pointing
  ...
  /* now compute the transformation matrix for a coordinate system with
     the donor at the origin, the Y-axis is defined by the direction of
     either the pre-donor atom, or, if there are two neighbors, by the
     average of the two, the X-axis is either defined by the acceptor
     position, or, if there are two pre-donors, by one of them */
  ...
  switch ( which_case )
	...
	* cos ( ( 180.0 - angle ) * M_PI / 180.0 ) / xpoint[Y];
    ...
    case STRAIGHT_IN_PLANE:
      /* Hydrogen is located directly on the Y-axis */
      hydrogen_position[X] = 0.0;
      hydrogen_position[Y] = bond_length; 
      hydrogen_position[Z] = 0.0;
      break;
    case TETRAHEDRAL_2_NEIGHBORS:
      /* two positions for the hydrogen are possible */      
      /* better don't mess around with the original acceptor entry... */
      for ( i = 0; i < 3; i++ )
				       ...
				       global_data_pt global)
{
  ...
  if ( atoms[index].type == N )
	   ...
	   and C in previous residue */
	{
	  ...
	  else  /* in case we don't have other atoms in current residue except this 'N' due to
		 the crystallographic result , we use 'C' and 'O' in the previous residue
		 and current 'N' to form the plane      --- added by Litian He   03/17/2004 */
	      {
		  atom_types[0] = C;
		  atom_types[1] = O;
		  find_plane = get_positions(&atoms[residues[residue_index].start_atom],
			    ...
			    float          *angle )
{
  ...
  if ( ligand->atoms[ligand_index].act == ACCEPTOR
	...
	compute_target_hydrogen_angle ( global->target_atoms,
      ...
      /* toneroma 04OCT07 adding code to check pre-acceptor angle */
      ...
      for ( i = 0; i < ligand->number_of_neighbors[ligand_index]; i++ )
	{
      ...
      /* toneroma 04OCT07 end adding code to check pre-acceptor angle */

      /*      printf("\ntoneroma angle: %f\n", *angle);        toneroma 25MAY06 - debugging*/
      /* check, if the hydrogen points towards the acceptor, in this case 
	 only this particular hydrogen bond can be formed, so 'deactivate' 
	 the donor for later hbond -- as of 31MAY06 I DISABLED this check,
	 since no other case deactivates hbonding potential, and we want 
	 to count all potential hbonds - toneroma 31MAY06
      if ( *angle > MIN_HYDROGEN_ANGLE 
	   && *angle < MAX_HYDROGEN_ANGLE 
	   && ( ( target_atom->res == SER 
		  && target_atom->type == OG )
		|| ( target_atom->res == THR
		     && target_atom->type == OG1 )
		|| ( target_atom->res == TYR
		     && target_atom->type == OH ) ) ){
		     target_atom->act = ACCEPTOR;}*/

    /* the donor is a ligand atom */
  }else{
                    ...
                    float *angle)
{
   ...
   * unlikely to be used anytime soon  -- no Sulfur hbonds in SLIDE at present
   */
