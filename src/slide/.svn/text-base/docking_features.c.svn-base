                         ...
                         int docking)
{
  ...
  fprintf(fout, "%s: O: %5.3f A/N: %5.3f A: %5.3f [%-5.3f %-5.3f %-5.3f] "
          "%4.0f %3d %3d %3d %3d %3d %5.3f", lig_fname,
          features->orient_score, features->affiefficient_score,
                           ...
                           atom_pt ligand_atoms, atom_pt target_atoms)
{
  ...
  fprintf(fp, "# SLIDE Version: %s\n", VERSION);
  fprintf(fp, "# Time stamp: %s\n", slide_get_local_time(time_stamp, 80));
  ...
  fprintf(fp, "# Orientscore (kcal/mol), used by SLIDE to select binding "
...
/* Jeff Van Voorst -- Sept 10, 2008
 * As far as I can tell the scoring of water mediated hbonds was not completed.
 * At least there was (and still is) no record keeping of the water atom(s)
 * mediating the water mediated hbonds.
 */
    else
      
    fprintf ( fp, 
		"# hbond %3d: %3d  %-5s -- water %3d (dist: %5.3f, D-H-A angle: %3.1f)\n",
		      i+1,  /* toneroma 24MAY06 - added 1 so that numbering start at 1, not at 0*/
