/*
 *
 *   count_flexible_bonds.c    Litian He   Tue April 27 2004
 *   
 *   functions:  count_flexible_ligand_bonds ()
 *               count_flexible_target_bonds ()
 *
 *   look for bonds that satisfy the following criteria:
 *           1. flexible: defined in find_flexible_bonds.c
 *           2. interfacial:   if any atom in the bond is interfacial.
 *                             defined in ligand_flag[] in score_complex.c
 *           3. non-terminal:  have more than 1 neighbor
 *           4. any connected atom is not CHn*
 */
                                   ...
                                   short           ligand_flag[MAX_LIGAND_ATOMS] )
{
...
#ifdef TRACE
    printf("count_flexible_ligand_bonds:\n");
#endif
    ...
    for ( i = 0; i < molecule->number_of_bonds; i++ )
    {
        ...
        /* find the rotatable, interfacial bonds, and non-terminal */
        ...
        {
            /* any atom in the bond can't be Carbon that only has hydrogen neighbors
               other than this neighbor  (a terminal CH , CH2 or CH3) */
            /*printf(" atoms[%d].name = %s  ", atom1,  atoms[atom1].name);*/
            ...
            if ( atoms[atom1].name[0] == 'C' )
            {
                ...
                {
		  /*printf("molecule->neighbors[%d][%d] = %d %s\n",
                    atom1, j, molecule->neighbors[atom1][j],
		    atoms[molecule->neighbors[atom1][j]].name );*/
            ...
            /*printf(" atoms[%d].name = %s  ", atom2,  atoms[atom2].name);*/
                ...
                atoms[atom2].name[0] == 'C' )
            {
                ...
                {
		  /*printf("molecule->neighbors[%d][%d] = %d %s\n",
		    atom2, j, molecule->neighbors[atom2][j],
		    atoms[molecule->neighbors[atom2][j]].name );*/
            ...
            if ( only_hydrogen_neighbors == 0 )
            {   /* so far both atom1 and atom2 satisfy our criteria. */
...
/*             printf("bond %d: %d - %d\n", i, */
/*                 global->atoms->bonds[i].atom1, global->atoms->bonds[i].atom2 ); */
#endif          
            }
        }
#ifdef TRACE3
...
/* for target bonds, we only check those interfacial, rotatable, non-terminal, non-CHn
 bonds on side-chains */
...
#endif
            /* check the last residue. we assume atoms in one residue appear sequencially in PDB file */
