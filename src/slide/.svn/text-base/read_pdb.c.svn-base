/*
 * $Source: /psa/share/repository/slide/src/slide/read_pdb.c,v $
 * $Revision: 1.11 $
 * $Author: vanvoor4 $
 * $Date: 2009/03/19 14:53:32 $
 * 
 * $Log: read_pdb.c,v $
 * Revision 1.11  2009/03/19 14:53:32  vanvoor4
 * Changed comment for OXT to be a bit more readible
 *
 * Revision 1.10  2009/03/12 16:55:07  vanvoor4
 * typo -- should be atm not atom
 *
 * Revision 1.9  2009/03/12 15:14:57  vanvoor4
 * We did not have a hydro value for OXT for any of the residues
 *
 * Revision 1.8  2009/02/26 21:08:31  vanvoor4
 * No need to pass global to err print
 * Grab the atom number -- don't want uninitialized fields
 *
 * Revision 1.7  2008/11/06 16:05:06  toneroma
 * Added setting metal atm->level to HETATM to fix bug that caused a seg fault when compiled with gcc 4.1.2 and  if metals involved in a bump.
 *
 * Revision 1.6  2008/09/16 20:27:47  toneroma
 * Changes to properly handle insertion codes in pdb files
 *
 * Revision 1.5  2008/09/02 15:15:55  toneroma
 * change to remove unused variable
 *
 * Revision 1.4  2007/09/28 18:33:48  toneroma
 * *** empty log message ***
 *
 *
 */ 
/* 
 *   read_pdb.c      Volker Schnecke    Wed Apr 16 21:09:37 EDT 1997
 *
 *   function:  read_pdb()    reads pdb-file and fills data-vectors
 *              assign_type_and_radius()
 *                            assigns type and radius to a given atom
 *              assign_residue_type()
 *                            assigns type of residue to a given atom
 *              read_header() reads NAME, CMPND, and TITLE entries
 */
...
/*  Function 'read_pdb()' reads in atoms contained in file 'fname', inserts 
 *  atom-data into vector 'atoms', interaction points into vector 
 *  'interactions', and all residues into vector 'residues'.
 *  All vectors have to be allocated before calling this function.
 *  If more than 'MAX_PDB_ATOMS' atoms or interaction-points or more
 *  than 'MAX_PDB_RESIDUES' are read from the pdb-file, then the function
 *  is exited via 'err_panic()'
 */
	   ...
	   && 1 == sscanf ( column ( 14 ), " %3s", atm->name ) )
	{
	  ...
	  if(strkeq ( "ATOM", linebuf, 4 ) || strkeq ( "atom", linebuf, 4 )){ 
	    ...
	    atm->hydro = hydro[atm->res][atm->type];
            /* 99% sure that OXT will always get 0 as hydro 
             * Instead, we set it to the default value (for oxygen)
		...
		handle the metal HETATM to count the metal hbond in scoring
		   ...
		   *column ( 19 ) == 'C' && *column ( 20 ) == 'A' )
		 {
	      ...
	      if ( *column ( 13 ) == 'F' && *column ( 14 ) == 'E' && *column ( 15 ) == ' ' )
		  {
		      ...
		      atm->type = FE;
		      atm->act = METAL_2;
		      atm->hydro = 317;  /*  Sameer - is this deliberate or oversight ???*/
		      atm->hydro = 635;
		      atm->level = HETATM;
		      strcpy(atm->type_str,  "Fe");
#ifdef TRACE_METAL
	  ...
	  /* The following code is commented out.
	     As PDB files usually don't contain charges, we assign charge
	     manually to atoms upon atom types and residues. */

	  /* see, if there are any charges (1+,1-,2+,2-) assigned */
	  /* 	  if ( strlen ( linebuf ) > 78 ) */
	  /* 	    { */
	  /* 	      if ( strncmp ( linebuf + 78, "1+", 2 ) == 0 ) */
	  /* 		atm->charge = 1.0; */
	  /* 	      else if ( strncmp ( linebuf + 78, "1-", 2 ) == 0 ) */
	  /* 		atm->charge = -1.0; */
	  /* 	      else if ( strncmp ( linebuf + 78, "2+", 2 ) == 0 ) */
	  /* 		atm->charge = +2.0; */
	  /* 	      else if ( strncmp ( linebuf + 78, "2-", 2 ) == 0 ) */
	  /* 		atm->charge = -2.0; */
	  /* 	    } */
	  ...
	  /*****************************************************
                   Assign charges to certain specified atoms
                              added by Litian He   01/08/2004 
	  ******************************************************/
	  assign_charge ( atm );
...
/* The following was added as a check for whether the PDB file is being read correctly */
