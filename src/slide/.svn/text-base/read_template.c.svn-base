/*
 *   read_template.c    Volker Schnecke  Wed Apr 16 22:05:59 EDT 1997
 *
 *   functions:  read_template()   reads description of interaction points
 *               find_atom_index() searches atoms in global data structure
 */
...
int read_template(char           *filename, global_data_pt global )
{
  ...
  while ( fgets ( line, MAX_TEMPLATE_LINELENGTH, in ) != NULL ){
    ...
    if ( act[0] == 'A' ) {
      inter->act = ACCEPTOR;
      inter->ref = number_of_read_points;  /* toneroma 26SEP06*/
    } if ( act[0] == 'D' ) {
      inter->act = DONOR;
      inter->ref = number_of_read_points;  /* toneroma 26SEP06*/
    } if ( act[0] == 'H' ) {
