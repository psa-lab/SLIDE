				      ...
				      transform_matrix_t matrix )
{
  ...
  /*  printf("\nJeff stuff\n");   toneroma 30MAY06 - debugging
  for(i = 0; i < 3; i++) printf("%f ", axis[i]);
  printf("\n");
  for(i = 0; i < 3; i++) printf("%f ", angle[i]);
  printf("\n");
  */
  /* first rotate CA-CB-axis around the X-axis to the XY-plane */
  if ( axis[Y] == 0.0 ) angle[X] = M_PI/2;  
  /* toneroma 30MAY06 - angle should not be set to 0.0, as was previous.  Did not want to pass axix[Y]==0.0 into the next function in order to avoid a divide by 0.  atan of (a number / 0) is PI/2.*/
  else{
  ...
  if ( axis[Y] * cos ( angle[X] ) - axis[Z] * sin ( angle[X] ) == 0.0 )
    angle[Z] = M_PI/2;  /* toneroma 30MAY06 - angle should not be set to 0.0, as was previous.  Did not want to pass axix[Y]==0.0 into the next function in order to avoid a divide by 0.  atan of (a number / 0) is PI/2.*/
  else {
  ...
  if ( help[X] * cos ( angle[Z] )
    ...
    angle[Y] = M_PI/2;  /* toneroma 30MAY06 - angle should not be set to 0.0, as was previous.  Did not want to pass axix[Y]==0.0 into the next function in order to avoid a divide by 0.  atan of (a number / 0) is PI/2.*/
  else
    {
...
/* we only keep the first 4 digits in the decimal fraction so that
   both PC and SGI get to the same output*/
float cut (float f)
{
    int temp;
			...
			transform_matrix_t matrix )
{
  ...
  /* print statement for debugging - toneroma 30MAY06
  for ( i = 0; i < 4; i++ )
    for (j = 0; j < 4; j++)
      printf("\nMatrix %d %d: %f\n", i, j, matrix[i][j]);
*/
