                        ...
                        int number_of_rules)
{
  ...
  sprintf ( file, "./flex.log");	
  flex_log = fopen(file,"a+");
  fprintf ( flex_log, 
	    "ligand: %s\n", &molecule->name);
	    fclose(flex_log);*/
  ...
  for( i = 0; i < molecule->number_of_bonds; i++ ){
      ...
      for ( j = 0; j < MAX_RULES_PER_BONDED_ATOM && result2 == SUCCESS; j++ )
        if ( bond2->required[j] != NULL ) 
          result2 = check_required_rule(molecule, atom2, atom1, 
                                        bond2->required[j] );
      /* toneroma 07MAR07
       * we must pass of the cases, i.e. only discard those where
       * we fail in either, result1 and result2 */
    ...
    if(molecule->bonds[i].type == FLEX)
      molecule->flexible_bonds[molecule->number_of_flexible_bonds++] = i;
	      /*
	         if ((molecule->atoms[molecule->bonds[i].atom1].type != H) &&  (molecule->atoms[molecule->bonds[i].atom2].type != H))
		{
		  sprintf ( file, "./flex.log");
		  flex_log = fopen(file,"a+");
		  fprintf ( flex_log, 
			    "%s (%s) :: %s (%s) FLEXIBLE %d\n", &molecule->atoms[molecule->bonds[i].atom1], &molecule->atoms[molecule->bonds[i].atom1].type_str, &molecule->atoms[molecule->bonds[i].atom2], &molecule->atoms[molecule->bonds[i].atom2].type_str, rule_used);
		  fclose(flex_log);
		  printf ("%s (%s) :: %s (%s) FLEXIBLE %d\n", &molecule->atoms[molecule->bonds[i].atom1], &molecule->atoms[molecule->bonds[i].atom1].type_str, &molecule->atoms[molecule->bonds[i].atom2], &molecule->atoms[molecule->bonds[i].atom2].type_str, rule_used);
		}	      
*/
